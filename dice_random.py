...
class DiceRandom(ExplainerBase):
    ...
    def _generate_counterfactuals(self, query_instance, total_CFs, desired_range=None,
        ...
        for num_features_to_vary in range(1, len(self.features_to_vary)+1):
            ...
            print("Candidate cfs shape:", candidate_cfs.shape)
            print("query_instance:", query_instance.shape)
            print("Candidate cfs columns:", candidate_cfs.columns.tolist())
            print("query_instance columns:", query_instance.columns.tolist())
